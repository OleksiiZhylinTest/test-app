...
| JFM-D-002 | The default filter pre-sets sensible parameter defaults | The default entry has `JIRA_INCLUDE_ACTIVE_SPRINT=true` and `schema_name=Default_Jira_Cloud`; `JIRA_PROJECT` is intentionally blank (instance-specific) | ✓ Met | — |
...
| JFM-P-006 | `POST /api/filters` builds correct JQL from params | The generated JQL always includes `status = Done`, plus `project =`/`IN`, optional `Team[Team] =`/`IN`, optional `type IN`, and `sprint in closedSprints()` when `JIRA_INCLUDE_ACTIVE_SPRINT` is truthy | ✓ Met | — |
