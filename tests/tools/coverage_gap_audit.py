...
def analyze_test_files(source_modules: list[Path]) -> list[TestFileInfo]:
    ...
    variants_map: dict[str, set[str]] = {
        module_key(m): module_stem_variants(m) for m in source_modules
    }
...
def build_coverage_matrix(
    source_modules: list[Path], test_files: list[TestFileInfo]
) -> list[ModuleCoverage]:
    cov_map: dict[str, ModuleCoverage] = {
        module_key(m): ModuleCoverage(key=module_key(m)) for m in source_modules
    }
...
def print_matrix(matrix: list[ModuleCoverage]) -> None:
    ...
    for mc in matrix:
        cols = " ".join(
            ("Y" if mc.covered_at(L) else "-") for L in ["unit", "component", "integration", "e2e"]
        )
